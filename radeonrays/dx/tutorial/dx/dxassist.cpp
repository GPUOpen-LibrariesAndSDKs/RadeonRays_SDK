...
ComPtr<ID3D12GraphicsCommandList> DxAssist::CreateCommandList(ID3D12CommandAllocator* command_allocator)
{
...
ComPtr<ID3D12CommandAllocator> DxAssist::CreateCommandAllocator()
{
...
ComPtr<ID3D12Fence> DxAssist::CreateFence()
{
...
ComPtr<ID3D12Resource> DxAssist::CreateUploadBuffer(UINT64 size, void* data)
{
...
ComPtr<ID3D12Resource> DxAssist::CreateUAVBuffer(UINT64 size, D3D12_RESOURCE_STATES initial_state)
{
    ComPtr<ID3D12Resource> resource        = nullptr;
...
ComPtr<ID3D12Resource> DxAssist::CreateReadBackBuffer(UINT64 size)
{
    ComPtr<ID3D12Resource> resource        = nullptr;
...
void DxAssist::InitDXGI(D3D_FEATURE_LEVEL feature_level)
{
    ...
    // Get required experimental features
    std::vector<IID> required_experimental_features;
    required_experimental_features.emplace_back(D3D12ExperimentalShaderModels);

    ThrowIfFailed(D3D12EnableExperimentalFeatures(static_cast<UINT>(required_experimental_features.size()),
                                                  required_experimental_features.data(),
                                                  nullptr,
                                                  nullptr),
                  "Cannot enable experimental features");
