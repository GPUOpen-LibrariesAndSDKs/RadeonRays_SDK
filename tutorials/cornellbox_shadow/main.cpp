...
int main(int argc, char* argv[])
{
    ...
    for (int i = 0; i < g_window_height; ++i)
        for (int j = 0; j < g_window_width; ++j)
        {
            ...
            shadow_rays[k].SetMaxT(std::sqrt(dir.sqnorm()) - std::numeric_limits<float>::epsilon());

        }

    int* tmp_occl1 = new int[k_raypack_size];
    for (int i = 0; i < k_raypack_size; ++i)
    {
        tmp_occl1[i] = -1;
    }
    Buffer* shadow_rays_buffer = api->CreateBuffer(shadow_rays.size() * sizeof(ray), shadow_rays.data());
    Buffer* occl_buffer = api->CreateBuffer(k_raypack_size * sizeof(int), tmp_occl1);
    ...
    for (int i = 0; i < k_raypack_size ; ++i)
    {
        ...
        if (shape_id != kNullId && prim_id != kNullId)
        {
            ...
            if (tmp_occl[i] != kNullId && tmp_occl[i] != shape_id)
            {
                continue;
            }
